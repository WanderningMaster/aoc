...
func validatePair(rangePair RangePair) bool {
	var longer, shorter Range
	if rangePair.first.len() > rangePair.second.len() {
		longer = rangePair.first
		shorter = rangePair.second
	} else {
		shorter = rangePair.first
		longer = rangePair.second
	}

	fullContains := longer.start <= shorter.start && longer.end >= shorter.end
	if fullContains {
		return false
	}
