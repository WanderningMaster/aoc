...
const FS_SIZE int = 70_000_000
const UPDATE_SIZE int = 30_000_000

func smallestDirToDelete(node *TreeNode, minSize int, smallest int) int {
	if node.Type == Dir {
		dirSize := dirTotalSize(node)
		if minSize <= dirSize && dirSize < smallest {
			smallest = dirSize
		}
	}
	for _, node := range node.children {
		smallest = smallestDirToDelete(node, minSize, smallest)
	}
...
func main() {
	...
	minSize := dirTotalSize(root) - (FS_SIZE - UPDATE_SIZE)
	smallestSizeToDelete := dirTotalSize(root)

	fmt.Println("Result:", smallestDirToDelete(root, minSize, smallestSizeToDelete))
}
