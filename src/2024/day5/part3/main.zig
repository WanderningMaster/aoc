...
const Graph = @import("internal").GraphSet;
...
fn solve(allocator: std.mem.Allocator, log: Logger, manual: Manual) !u64 {
    ...
    for (manual.updates.items) |update| {
        ...
        for (0..update.len) |currIdx| {
            const rules = manual.rules.adjacency_set[@intCast(update[currIdx])];

            var found: usize = 0;
            for (0..update.len) |idx| {
                if (currIdx == idx) continue;

                if (rules.contains(update[idx])) {
                    found += 1;
                }
            }
...
// Solution with hashset instead of arraylist
pub fn main() !void {
