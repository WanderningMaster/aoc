...
const Map = struct {
    ...
    pub fn out_of_boundary(self: *Map, p: Point) bool {
        return (p.x > self.boundaries.x or p.x < 0 or p.y > self.boundaries.y or p.y < 0);
    }
    ...
    while (map.antennas.next()) |antennas| {
        for (0..antennas.len) |i| {
            for (i + 1..antennas.len) |j| {
                ...
                var a1 = p1;
                while (!map.out_of_boundary(a1)) {
                    try map.push_antinode(a1);
                    a1 = a1.add(distance);
                }

                var a2 = p2;
                while (!map.out_of_boundary(a2)) {
                    try map.push_antinode(a2);
                    a2 = a2.sub(distance);
                }
            }
        }
